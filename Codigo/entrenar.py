...
tl.files.load_and_assign_npz(sess=sess, name='nEs.npz', network=net)
...
for epoch in range(n_epoch):
    ...
    for X, Y in tl.iterate.minibatches(inputs=trainX, targets=trainY, batch_size=batch_size, shuffle=False):
        ...
        #if n_iter % 1000 == 0:
    if epoch % 2 == 0:
        seeds = ["amigos de ibk les he dejado un mensaje por inbox para que me ayuden",#0
                 "pueden responder es urgente", #8
                 #17
                 #"srs interbank mi dni es 0821145 me han robado 8000 dollares después que mas vaciaron recien me avisa el sr cacique a las 8 de la noche siendp el primer de retiro a las dos de la tarde tengo más de un sr cacique estaremos en imterleche cuidado con sus ahorros este banco no te protege",
                 #20
                 #"hoy hasta que hora atienden en el mall del sur xfa"
                 "Me han robado, que puedo hacer",
                 "la banca movil no dunciona",
        ...
        for seed in seeds:
            ...
            #seed = reducirPalabra(seed)
            seed_id = [w2idx[w] for w in seed.split(" ")]
            for _ in range(2):  # 1 Query --> 5 Reply
                # 1. encode, get state
    ...
    if epoch % 2 == 0:
        tl.files.save_npz(net.all_params, name='nEs.npz', sess=sess)
    if epoch==30:
        tl.files.save_npz(net.all_params, name='nEs30.npz', sess=sess)
    if epoch==40 == 0 and epoch>1:
        tl.files.save_npz(net.all_params, name='nEs40.npz', sess=sess)
    if epoch%50 == 0 and epoch>1:
        tl.files.save_npz(net.all_params, name='nEs50.npz', sess=sess)
    if epoch%60 == 0 and epoch>1:
        tl.files.save_npz(net.all_params, name='nEs60.npz', sess=sess)
    if epoch%70 == 0 and epoch>1:
        tl.files.save_npz(net.all_params, name='nEs70.npz', sess=sess)
    if epoch%80 == 0 and epoch>1:
        tl.files.save_npz(net.all_params, name='nEs80.npz', sess=sess)
    if epoch%90 == 0 and epoch>1:
        tl.files.save_npz(net.all_params, name='nEs90.npz', sess=sess)
