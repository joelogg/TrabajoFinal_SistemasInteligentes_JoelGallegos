...
def cambiarDesconocidos(oracion, metadata):
    
    w2idx = metadata['w2idx']
    oracionSplit = oracion.split(" ") 
    tam = len(oracionSplit)
    nuevaOracion = ""
    
    for i in range(tam-1):
        idGet = oracionSplit[i] in  w2idx
        if idGet == False:
            nuevaOracion = nuevaOracion + 'unk' + ' '
        else:
            nuevaOracion = nuevaOracion +oracionSplit[i] + ' '
    
    
    idGet = oracionSplit[tam-1] in w2idx
    if idGet == False:
        nuevaOracion = nuevaOracion + 'unk' 
    else:
        nuevaOracion = nuevaOracion +oracionSplit[tam-1] 
...
while 1:
    ...
    seed = cambiarDesconocidos(seed, metadata)
